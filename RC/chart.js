async function drawRadar() {
  ...
  // 6. Draw peripherals
  // We're drawing our axes early here : don't overlap our radar line
  ...
  const gridLines = metrics.map((metric, i) => {
    ...
    return axis
      ...
      .attr(
        "x2",
        Math.cos(angle) * dimensions.boundedRadius + dimensions.boundedWidth / 2
      )
      .attr("y1", dimensions.boundedHeight / 2)
      .attr(
        "y2",
        Math.sin(angle) * dimensions.boundedRadius + dimensions.boundedWidth / 2
      )
      .attr("class", "grid-line");
  });
